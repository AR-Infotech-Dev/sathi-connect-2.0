...
//   cleanupRuntimeData(dataDir, logDir);
...
// function cleanupRuntimeData(dataDir, logDir) {
//   const dataFilesToRemove = [
//     "sathi-response-archive.json"
//   ];
//   const dataPatternsToRemove = [
//     /^createOrder-.+\.json$/i,
//     /^get[A-Za-z0-9_-]*-.+\.json$/i,
//     /^pull[A-Za-z0-9_-]*-.+\.json$/i,
//     /^fetch[A-Za-z0-9_-]*-.+\.json$/i
//   ];

//   for (const fileName of dataFilesToRemove) removeFileIfExists(path.join(dataDir, fileName));
//   removeMatchingFiles(dataDir, dataPatternsToRemove);
//   removeMatchingFiles(logDir, [/./]);
// }

// function removeMatchingFiles(directory, patterns) {
//   try {
//     if (!fs.existsSync(directory)) return;
//     for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
//       if (!entry.isFile()) continue;
//       if (!patterns.some((pattern) => pattern.test(entry.name))) continue;
//       removeFileIfExists(path.join(directory, entry.name));
//     }
//   } catch (error) {
//     logMain("runtime.cleanup.matching_files.failed", { directory, error: error.message });
//   }
// }
...
//   } catch (error) {
//     logMain("runtime.cleanup.file.failed", { filePath, error: error.message });
//   }
// }
...
async function createWindow() {
  try {
    ...
    mainWindow.on("focus", () => {
      mainWindow.webContents.executeJavaScript("window.dispatchEvent(new Event('sathi-window-focus'))").catch(() => {});
    });
...
async function ensureServer() {
  ...
  configureRuntimePaths();
  const { startServer, serverFile } = await loadServerModule();
  logMain("server.import.loaded", { serverFile });
  const started = await startServer({ port: PORT, silent: true });
  localServer = started.server;
...
async function loadServerModule() {
  const bundledServerFile = resolveRuntimeFile("electron", "server.bundle.cjs");
  if (fs.existsSync(bundledServerFile)) {
    return {
      serverFile: bundledServerFile,
      ...require(bundledServerFile)
    };
  }
...
function configureRuntimePaths() {
  ...
  process.env.SATHI_ENV_FILE = packagedEnvFile;
  process.env.SAATHI_RESOURCES_FILE = path.join(runtimeRoot, "config", "resources.example.json");
  process.env.SATHI_DATA_DIR = dataDir;
  process.env.SATHI_LOG_DIR = logDir;
  process.env.SATHI_DB_FILE = path.join(dataDir, "saathi-connect.sqlite");
  process.env.SATHI_PUBLIC_KEY_FILE = path.join(runtimeRoot, "keys", "public.pem");

...
const buildArch = builderArgs.includes("--x64") ? "x64" : "ia32";
const buildDir = path.join(rootDir, "build", `protected-app-${buildArch}-${Date.now()}`);
...
async function bundleServer() {
  const entryFile = path.join(buildDir, "src", "server.js");
  const outputFile = path.join(buildDir, "electron", "server.bundle.cjs");
  await esbuild({
...
function removeBundledSource() {
  for (const entry of ["src", "portable-license-flow"]) {
    const target = path.join(buildDir, entry);
    if (!fs.existsSync(target)) continue;
    assertInside(buildDir, target);
...
function organizeFinalOutput() {
  ...
  const sourceInstaller = path.join(outputDir, `Sathi Connect Setup ${rootPackage.version} ${installerArch}.exe`);
  ...
  const finalInstaller = path.join(finalSetupDir, `Sathi Connect Setup ${rootPackage.version} (${bitLabel}).exe`);
...
function removeIntermediateOutput(arch) {
  const names = [
    ...
    `Sathi Connect Setup ${rootPackage.version} ${arch === "x64" ? "x64" : "ia32"}.exe`,
    `Sathi Connect Setup ${rootPackage.version} ${arch === "x64" ? "x64" : "ia32"}.exe.blockmap`,
    `saathi-setu-${rootPackage.version}-${arch}.nsis.7z`,
    "builder-debug.yml",

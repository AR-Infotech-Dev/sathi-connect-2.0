...
export function readArchive() {
  ...
  try {
    const rawArchive = JSON.parse(fs.readFileSync(filePath, "utf8"));
    const archive = Array.isArray(rawArchive) ? rawArchive.map(sanitizeArchiveEntry) : [];
    if (JSON.stringify(rawArchive) !== JSON.stringify(archive)) {
      fs.mkdirSync(DATA_DIR, { recursive: true });
      fs.writeFileSync(ARCHIVE_FILE, JSON.stringify(archive.slice(0, MAX_ARCHIVE_ITEMS), null, 2));
    }
    return archive;
  } catch {
    return [];
...
export function saveSathiResponse(action, request, response) {
  const archive = readArchive().map(sanitizeArchiveEntry);
  const entry = {

...
export class SaathiClient {
  ...
  async request(endpoint, options = {}) {
    ...
    if (!response.ok) {
      throw new Error(`Saathi request failed: HTTP ${response.status} ${JSON.stringify(response.data)}`);
    }
...
function requestJson(url, options) {
  return new Promise((resolve, reject) => {
    ...
    request.on("timeout", () => {
      request.destroy(new Error(`Saathi request timed out after ${options.timeoutMs || 30000} ms.`));
    });
